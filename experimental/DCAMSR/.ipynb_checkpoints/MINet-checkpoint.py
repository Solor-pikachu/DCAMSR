...
class SRModule(MriModule):
    ...
    def forward(self, Ref,Ref_SR,LR):
        pd, pdfs = self.network(LR.unsqueeze(1),Ref.unsqueeze(1),Ref_SR.unsqueeze(1))
        pdfs = pdfs.squeeze(1)
        pd = pd.squeeze(1)
        return pd, pdfs
    ...
    def training_step(self, batch, batch_idx):
        ...
        pd, pdfs = self(targetpd,imagepd,image)
    ...
    def configure_optimizers(self):        
        optim = torch.optim.Adam(self.parameters(), lr=1e-5*4, weight_decay=0.0, betas=(0.9, 0.999), eps=1e-8)
        scheduler = torch.optim.lr_scheduler.StepLR(
            optim, 100, self.lr_gamma,verbose=True
        )
